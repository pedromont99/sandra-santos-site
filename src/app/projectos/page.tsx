...
export default function Projetos() {
  return (
    <main className="pt-32 pb-20 bg-[#F8F5F2] min-h-screen font-sans">
      <div className="max-w-7xl mx-auto px-6 md:px-12">

        {/* CABEÇALHO EDITORIAL */}
        <div className="mb-24 border-b border-[#382622]/10 pb-12">
          <p className="text-[#59310b] text-xs font-bold uppercase tracking-[0.4em] mb-6">Portefólio</p>
          <h1 className="text-5xl md:text-8xl font-bold text-[#382622] tracking-tighter italic font-serif">
            Projetos <span className="text-[#59310b] not-italic font-sans underline decoration-1 underline-offset-8">Selecionados</span>.
          </h1>
        </div>
            ...
            <div key={proj.id} className="group relative">
              <div className="grid lg:grid-cols-12 gap-12 items-start">
                ...
                <div className="lg:col-span-4 space-y-8 order-2 lg:order-1">
                  <span className="text-6xl font-serif italic text-[#D4A373]/30 block">{proj.id}</span>

                  <div>
                    <h2 className="text-3xl md:text-5xl font-bold text-[#382622] mb-4 tracking-tight">
                      {proj.cliente}
                    </h2>
                    <p className="text-[#59310b] font-bold text-sm uppercase tracking-widest mb-6">
                      {proj.categoria}
                    </p>
                  </div>

                  <p className="text-lg text-[#382622]/70 leading-relaxed max-w-sm">
                    {proj.descricao}
                  </p>

                  <div className="flex flex-wrap gap-3 pt-4">
                    {proj.tags.map((tag, i) => (
                      <span key={i} className="px-4 py-1 border border-[#382622]/10 rounded-full text-[10px] font-bold uppercase tracking-tighter text-[#382622]/50">
                        {tag}
                      </span>
                    ))}
                  </div>
                  ...
                  <Link href={`/projetos/${proj.id}`} className="inline-flex items-center gap-2 pt-8 group/link">
                    <span className="font-bold text-[#382622] border-b-2 border-[#59310b] pb-1">Ver Detalhes</span>
                    <ArrowUpRight className="w-5 h-5 text-[#59310b] group-hover/link:translate-x-1 group-hover/link:-translate-y-1 transition-transform" />
                  </Link>
                </div>
                ...
                <div className="lg:col-span-8 order-1 lg:order-2 space-y-8">
                  <div className="grid grid-cols-12 gap-6">
                    ...
                    <div className="col-span-6 md:col-span-4 relative aspect-square rounded-[2rem] overflow-hidden shadow-xl border border-[#382622]/5">
                      <Image
                    ...
                    <div className="col-span-6 md:col-span-4 bg-[#382622] p-8 rounded-[2.5rem] flex flex-col justify-center">
                      <span className="text-[#D4A373] text-[10px] font-bold uppercase tracking-widest mb-2">Insight</span>
                      <p className="text-[#F8F5F2] text-sm font-medium leading-relaxed italic font-serif">
                        "{proj.insight}"
                      </p>
                    </div>
        ...
        {/* CTA FINAL */}
        <div className="mt-40 text-center py-24 bg-[#382622] rounded-[3rem] text-[#F8F5F2] relative overflow-hidden">
          <div className="absolute top-0 left-0 w-64 h-64 bg-[#D4A373]/5 blur-[80px] -ml-20 -mt-20" />
          <div className="relative z-10 space-y-8">
            <h2 className="text-3xl md:text-6xl font-bold tracking-tighter">O seu projeto pode <br /> ser o próximo.</h2>
            <p className="text-[#F8F5F2]/60 max-w-lg mx-auto text-lg font-light">Vamos transformar a sua visão num posicionamento de autoridade inquestionável.</p>
            <Link href="/contacto" className="inline-block px-12 py-5 bg-[#D4A373] text-[#382622] rounded-xl font-bold hover:bg-[#F8F5F2] transition-all transform hover:scale-105 shadow-xl">
              Iniciar conversa
            </Link>
          </div>
        </div>
